...
I know that I want my test dataset to consist of images from at least one generator that is as state-of-the-art as I can find, so probably one that is MLLM-based. For my training dataset I want a variety of generator architectures, e.g GAN, diffusion and MLLM-based (but less modern ones than my test dataset). I think a good number to aim for is 1000 images per model type, i.e 1000 images generated by GANs, 1000 from diffusion models & 1000 from MLLMs. For the test set, I'd ideally like 1000 samples too. That means I'd need 4000 real images too (3000 for fine-tuning, 1000 for testing).

Note that there's no standard minimum number of images required for fine-tuning with LoRA. We'll take the approach of using as many images as we can find (within a reasonable amount of time) and aim to collect more data in the future depending on time availability.

I first explored the OpenFake[^openfake-dataset] dataset...relatively few MLLM-generated images. Point to exploratory notebook & summarise the findings discussed in the notebook.
...
Another content-related thing to consider is the SynthID watermark that Google's models add to images they generate. These models include Nano Banana & Imagen. Nano Banana was first released in August 2025, pro in November 2025 and the second version in February 2026. Imagen 4 & Imagen-4-Ultra are the latest Imagen models, released in May 2025. Images from these models should not be included in the fine-tuning dataset as the model could learn to detect the SynthID watermark.
...
- Even in the clean case, there's a claim you can't make: if your detector does well on Nano Banana, you can't conclude it "generalises to modern MLLM generators" in general, because part of that performance could rest on provider-specific signals (watermark or otherwise) rather than transferable generation artifacts. This is exactly why your explainability work earns its place — if Grad-CAM/attention maps on Nano Banana images light up on diffuse whole-image patterns rather than semantic regions, that's a hint the model may be reading something watermark-like; if they land on malformed hands or inconsistent lighting, that's the semantic detection you're after. The attribution maps become a diagnostic for whether you've accidentally built a watermark detector.
