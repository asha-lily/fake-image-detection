...
Liu et al addressed the limited receptive field of CNNs directly: alongside their analysis, they proposed Gram-Net, a CNN backbone augmented with "Gram layers" that compute global texture representations rather than relying on local convolutional features alone. This is an early acknowledgement that locality — the very property that makes CNNs good at spotting repeating artifacts — also constrains what they can see, a tension we'll return to when discussing transformer-based detectors.
...
The second key reconstruction method is AEROBLADE, which was developed by Ricker et al[^AEROBLADE] in 2024. This approach specifically targets latent diffusion models, in which the diffusion process runs inside the latent space of an autoencoder (AE). The key finding is that "generated images can be more accurately reconstructed by the AE than real images, allowing for a simple detection approach based on the reconstruction error". An interesting feature of this approach is that it doesn't require training a model (DIRE requires training a binary classifier on the error map). AEROBLADE was found to be "effective" on models that were state-of-the-art when the study was conducted, e.g Stable Diffusion and Midjourney. These are more recent than the models DIRE was tested on.


#### Vision transformers

- capture global / semantic features that survive transformations better than CNN-local features
- CLIP-feature-space approach in UniversalFakeDetect: using a frozen CLIP:ViT feature space generalises across generators far better than a trained-from-scratch CNN
- link to experiment motivation


#### Multi-modal

where fingerprint and reconstruction methods model the generator, the feature-space approaches (UniversalFakeDetect) instead model real images or use a frozen general-purpose feature space, which is what gives them their generalisation edge.
...
Given that many state-of-the-art multi-modal-LLM-based generators have a diffusion component, does this mean that DIRE works for detecting images generated by such models?
...
- "This limitation arises from classifiers being asymmetrically tuned to detect artifacts specific to training data"
...
- fine-tuned Qwen with SFT then RLHF & GRPO
- demonstrates strong robustness under compression, cropping, and downsampling
