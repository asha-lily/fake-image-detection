...
(? delete?) So, GANs -- specifically later GANs with convolutional layers (e.g StyleGAN & ProGAN) -- produce artifacts as a result of convolutional upsampling, and these can be detected by CNNs due to their focus on local features.





- 'spectral decay' artifact: all generators
- semantic & physical inconsistencies: all generators (but shrinking fast)
- VAE reconstruction signature: all latent models

- we've mentioned CNNs for detecting spectral artifacts. What about other architectures such as transformers?
- what detection techiques are actually being used today?


**Key papers**
- In 2023 Wang et al[^DIRE]
- CIFAKE: [^detect-paper3]
    - similar results to Baraheem [^detect-paper2]?
