...
Accuracy aside, the feature-space detection methods discussed in the previous section simply output a binary classification (real or fake), without providing any reasoning. The need for explainability motivates the use of multi-modal LLMs.

However, LLMs suffer from hallucinations. In a 2025 study, Ji et al[^detect-paper8] produced ~9000 AI-generated images annotated with captions explaining the visual flaws and bounding boxes showing where they're located. They fine-tuned MLLMs on this dataset, so that along with a binary label the model outputs predicted bounding boxes and a caption for each region. While high accuracy was reported, the human annotation required is labour-intensive.

The concern with supervised fine-tuning in this context is the inability to generalise to unseen artifacts. In the 2025 `ThinkFake` paper Huang et al pointed out that: "methods that incorporate MLLMs for explanation typically rely on extensive supervision and lack autonomous reasoning, making it difficult to generalise to complex, unseen scenarios". To address this, they use GRPO (group relative policy optimisation) reinforcement learning, which Huang et al say "enhances generalisation by letting the modl learn how to 'think'". 

In 2025 Ji et al designed 6 specialised prompts, each targeting a distinct visual or logical aspect of an image, for example 'defect query' for identifying abnormal lighting or textures and 'common sense reasoning' to detect physical or spatial inconsistencies[^detect-paper8]. The image is classified as real or fake by taking a majority vote acorss the 6 categories. As well as testing on images generated by GANs and diffusion models, they evaluated performance on





ThinkFake[^detect-paper7]
- forgery-reasoning prompt + GRPO: produces step-by-step reasoning and interpretable structured outputs
...
Towards Explainable Fake Image Detection with Multi-Modal Large Language Models[^detect-paper8] (2025)
...
- highlighted open problems: hallucination & misalignment with actual image content
...
- semantic inconsistency detection, explainability / reasonng, failure modes (hallucination, prompt-wording sensitivity, refusals)
    - semantic inconsistencies are disappearing as generators improve
